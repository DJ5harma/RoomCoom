...
1. Backend: `business-server\src\helper.ts` gives you a helper to send realtime signals.
...
2. Frontend: `web\plugins\useHelper.ts` gives you things like: `{
	members,
	easyApi,
...
(i): `easyApi`: to send requests to your `pnameRouter`, so `pnameRouter.get('/{sample}')` can be called like: `easyApi.get('/pname/sample')`
...
(ii) `subscribeSignal`: to listen to what you send from your plugin's backend code using `helper.sendSignal(req, stream, payload)`
- so use it like: `subscribeSignal(stream, (payload) => {//do anything here});`

(iii) `unsubscribeSignal`: to stop listening to the stream what you have subscribed to, use like `unsubscribeSignal(stream);`

(iv) `sourceId`: Don't worry about it for now, its the exact instance's (place's) id where your plugin runs and is used internally by my helper. It may be used in the future to make plugins interactive from instance to instance (like "Room1" to "Direct4") but this use-case seems unnecessary to me currently.